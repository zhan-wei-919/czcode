...
public class JwtService {
  ...
  private final SecretKey activeSigningKey;
  ...
  public JwtService(
      ...
      @Value("${auth.jwt.access-token-ttl-seconds:3600}") long accessTokenTtlSeconds,
      @Value("${auth.jwt.active-kid:" + DEFAULT_ACTIVE_KID + "}") String activeKid) {
    this.accessTokenTtlSeconds = accessTokenTtlSeconds;
    this.activeKid = normalizeKid(activeKid);

    Map<String, String> configuredKeys = Binder.get(environment)
        .bind("auth.jwt.keys", Bindable.mapOf(String.class, String.class))
        .orElseGet(Map::of);

    Map<String, SecretKey> signingKeys = loadSigningKeys(configuredKeys, secret, this.activeKid);
    this.activeSigningKey = signingKeys.get(this.activeKid);
    if (this.activeSigningKey == null) {
      throw new IllegalStateException("active kid key is not configured: " + this.activeKid);
    }
  ...
  public String generateAccessToken(UUID accountId, UUID userId, String email) {
    ...
    return Jwts.builder()
        ...
        .signWith(activeSigningKey, Jwts.SIG.HS256)
        .compact();
  }
  ...
  public Instant accessTokenExpiresAt(Instant issuedAt) {
    return issuedAt.plusSeconds(accessTokenTtlSeconds);
  }
